...
static struct line* addLine(size_t lineNumber)
{
	...
	memmove(&lines[lineNumber + 1], &lines[lineNumber], (linesUsed - lineNumber) * sizeof(struct line));
...
static void delete(void)
{
	...
	if (cursorX + windowX >= lines[y].length && y >= linesUsed - 1)
			return;
