...
static void copyFile(int sourceFd, const char* sourcePath, int destFd, const char* destPath);
static void copy(int sourceFd, const char* sourceName, const char* sourcePath,int destFd,
				const char* destName, const char* destPath, bool force, bool prompt, bool recursive);
...
int main(int argc, char* argv[])
{
		...
		switch (c)
		{
			...
			case '?':
				return help(argv[0], "[OPTIONS] SOURCE...DESTINATION\n"
							" -f, --force                force copy\n"
							" -i, --interactive          prompt brfore overwrite\n"
							" -R, -r, --recursive        recursively copy direntories\n"
	...
	if (optind == argc - 2)
	{
		...
		else if ((statResult < 0 && errno == ENOENT) || !S_ISDIR(destSt.st_mode))
		{
			copy(AT_FDCWD, argv[optind], argv[optind], AT_FDCWD, destination, destination, force, prompt, recursive);
			return status;
		}
	}
	...
	for (int i = optind; i < argc - 1; i++)
	{
		...
		copy(AT_FDCWD, source, source, destFd, destName, destPath, force, prompt, recursive);
		free(sourceCopy);
		free(destPath);
	}
}
        ...
        bool prompt, bool recursive)
{
    ...
    if (S_ISDIR(sourceSt.st_mode))
	{
        if (!recursive)
		{
            warnx("omitting directory '%s' because -R is not specified", sourcePath);
            status = 1;
            return;
        }
        if (destExists && !S_ISDIR(destSt.st_mode))
		{
            warnx("cannot overwrite '%s' with directory '%s'", destPath, sourcePath);
            status = 1;
            return;
        }
        if (!destExists)
        ...
        while (dirent)
		{
            ...
            char* newSourcePath = malloc(strlen(sourcePath) + strlen(dirent->d_name) + 2);
            if (!newSourcePath) err(1, "malloc");
            stpcpy(stpcpy(stpcpy(newSourcePath, sourcePath), "/"), dirent->d_name);
            char* newDestPath = malloc(strlen(destPath) + strlen(dirent->d_name) + 2);
